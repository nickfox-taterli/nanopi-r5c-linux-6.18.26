...
static int rknpu_iommu_pm_init(struct rknpu_device *rknpu_dev)
{
	...
	pdev = of_find_device_by_node(iommu);
	of_node_put(iommu);
	if (!pdev)
		return -EPROBE_DEFER;
	...
	rknpu_dev->iommu_dev = get_device(&pdev->dev);
	platform_device_put(pdev);

	return 0;
}

static void rknpu_iommu_pm_exit(struct rknpu_device *rknpu_dev)
{
	if (rknpu_dev->iommu_dev) {
		put_device(rknpu_dev->iommu_dev);
		rknpu_dev->iommu_dev = NULL;
	}
}
...
static int rknpu_power_on(struct rknpu_device *rknpu_dev)
{
...
#ifndef FPGA_PLATFORM
	if (rknpu_dev->vdd) {
		ret = regulator_set_voltage(rknpu_dev->vdd, 900000, 1350000);
		if (ret) {
			LOG_DEV_ERROR(
				dev,
				"failed to set vdd voltage for rknpu, ret: %d\n",
				ret);
			return ret;
		}
	...
	if (rknpu_dev->iommu_en && rknpu_dev->iommu_dev &&
	    !rknpu_dev->iommu_pm_active) {
		ret = pm_runtime_get_sync(rknpu_dev->iommu_dev);
		if (ret < 0) {
			LOG_DEV_ERROR(
				dev,
				"failed to get pm runtime for rknpu iommu, ret: %d\n",
				ret);
			goto out;
		}
		rknpu_dev->iommu_pm_active = true;
	}
...
static int rknpu_power_off(struct rknpu_device *rknpu_dev)
{
	...
	if (rknpu_dev->iommu_pm_active && rknpu_dev->iommu_dev) {
		pm_runtime_put_sync(rknpu_dev->iommu_dev);
		rknpu_dev->iommu_pm_active = false;
	}
